...
def train() -> np.array:
    ...
    resp = estat.data(
        "ENV_AIR_GGE",
        key={"unit": "THS_T", "freq": "A", "src_crf": "TOTX4_MEMONIA", "airpol": "GHG"},
    )
    emission_df = resp.to_pandas(datetime={"dim": "TIME_PERIOD"}).droplevel(
        level=["unit", "freq", "src_crf", "airpol"], axis=1
    )
    ...
    household_energy_df = resp.to_pandas(
    ...
    gas_df = resp.to_pandas(datetime={"dim": "TIME_PERIOD"}).droplevel(
        level=["unit", "freq", "siec", "nrg_bal"], axis=1
    )
    melted_gas_df = melt_smdx_dataframe(gas_df)

    merged_df = merge_dataframes(
        [melted_emissions_df, melted_household_energy_df, melted_gas_df]
    )
    merged_df.columns = ["year", "geo", "emissions", "energy", "gas"]
    merged_df = merged_df.drop(
        merged_df[(merged_df.geo == "EU27_2020") | (merged_df.geo == "EU20")].index
    )
    ...
    X = np.pad(
        standard_df.iloc[:, 1:3].to_numpy(dtype=np.float64),
        ((0, 0), (1, 0)),
        mode="constant",
        constant_values=1,
    )
...
def predict(feats: list[float], beta: list[float]) -> float:
    ...
    m = np.array(beta, dtype=np.float64)
    y_hat = np.matmul(x, m)
...
def merge_dataframes(dataframes: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    merged_df = reduce(
        lambda left, right: pd.merge(
            left, right, left_on=["year", "geo"], right_on=["year", "geo"]
        ),
        dataframes,
    )
    return merged_df
...
def fill_holes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for name, group in df.groupby("geo"):
        ...
        for i in range(1, len(group.columns)):
            ...
            x_present = np.pad(
                np.arange(len(d))[present_mask].reshape(-1, 1),
                ((0, 0), (1, 0)),
                mode="constant",
                constant_values=1,
            )
            ...
            x_missing = np.pad(
                np.arange(len(d))[missing_mask].reshape(-1, 1),
                ((0, 0), (1, 0)),
                mode="constant",
                constant_values=1,
            )
